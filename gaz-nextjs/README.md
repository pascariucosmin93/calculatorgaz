...
- cont utilizator (signup/login) cu parole hash-uite și salvate în Postgres
...
4. Containerul expune un endpoint de sănătate la `/api/health`, iar imaginea Docker include un `HEALTHCHECK` care îl interoghează pentru a confirma că Next.js și Prisma răspund corect.
...
1. Pornește atât aplicația, cât și Postgres + Nginx:
   ...
   Pentru link-ul de resetare parolă se folosește `RESET_PASSWORD_BASE_URL` (implicit configurat la
   `https://reset.gaz.cosmin-lab.cloud/`); poți suprascrie această variabilă pentru alte domenii.
   Dacă vrei să primești link-ul pe Discord, setează `DISCORD_RESET_WEBHOOK_URL` cu adresa webhook-ului tău.
